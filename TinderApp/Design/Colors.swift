//
//  Colors.swift
//  TinderApp
//
//  Created by Juri Ohto on 2021/10/25.
//

import UIKit

struct Colors {
    
    static var greenColor = UIColor.rgb(red: 137, green: 223, blue: 86)//green
    static var redColor = UIColor.rgb(red: 222, green: 110, blue: 110)//red
    static var pinkColor = UIColor.rgb(red: 227, green: 48, blue: 78)//pink
    static var lightPinkColor = UIColor.rgb(red: 227, green: 48, blue: 78, alpha: 0.2)//lightPink
    static var orangeColor = UIColor.rgb(red: 245, green: 208, blue: 108)//orange
    static var invalidButtonColor = UIColor.init(white: 0.7, alpha: 1)//gray
}
